...
//      continue;
        ...
        && it_per_id.solve_flag == 1 )
    {
      ...
      margin_pt.intensity = (float)it_per_id.weight;
          //(w_pts_i(0),w_pts_i(1),w_pts_i(2),it_per_id.weight);
...
//        point_cloud.header.stamp = ros::Time(estimator.Headers[i]);
//        point_cloud.header.frame_id = "world";
//        for (auto &it_per_id : estimator.f_manager.feature)
//        {
//            if(it_per_id.weight<0.5) continue;
...
//                int imu_j = i - it_per_id.start_frame;
...
void pubKeyframe(Estimator &estimator)
{
  ...
  if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
  {
    ...
    if(SHOW_IMAGE_WEIGHT)
    {
      ...
      if(estimator.imageBuf_0.empty() ||
         estimator.imageBuf_0.front().first!=estimator.Headers[i]) return;
    ...
    {
      int frame_size = it_per_id.feature_per_frame.size();
      if(it_per_id.start_frame < i && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
      {
        ...
        p_2d.values.push_back(it_per_id.feature_per_frame[imu_j].uv.y());
        p_2d.values.push_back(it_per_id.feature_id);
        p_2d.values.push_back(it_per_id.weight);
        point_cloud.channels.push_back(p_2d);
        if(SHOW_IMAGE_WEIGHT)
        {
          cv::Scalar color(0,255*it_per_id.weight,255*(1-it_per_id.weight));
          cv::Point pt = cv::Point(it_per_id.feature_per_frame[imu_j].uv.x(),
                                   it_per_id.feature_per_frame[imu_j].uv.y());
